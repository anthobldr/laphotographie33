...
export default function Hero({
  ...
  return (
    <>
      <div
        data-route={pathname}
        className={`${style.hero} d-flex flex-column`}
        style={
          pathname === "/"
            ? ({ "--hero-bg": `url(${activeBg})` } as React.CSSProperties)
            : undefined
        }
      >
              ...
              <div className="container-fluid pb-4">
                <div className="d-none d-lg-flex justify-content-center gap-5">
                  {caroussel.map((button) => (
                    ...
                    >
                      <span
                        className={`${style.carousselButton} ${activeCategory === button.id ? style.carousselButtonActive : ""}`}
                      >
                        {""}
                      </span>
                      {button.name}
